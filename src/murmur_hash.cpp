...
#define BIG_CONSTANT(x) (x##LLU)
    ...
    FORCE_INLINE static uint64_t getblock64( const uint64_t * p, int i )
    {
        return p[ i ];
    }
    ...
    uint32_t murmur3_hash_32( const void * key, uint32_t len,
                              uint32_t seed )
    {
    ...
    // Renamed from MurmurHash3_x64_128, optimized for 64 processors
    uint64_t murmur3_hash_64( const void * key, uint32_t len,
                              uint64_t seed )
    {
        const uint8_t * data = (const uint8_t*)key;
...
//        ( (uint64_t*)out )[ 1 ] = h2;
    ...
    uint64_t murmur3_hash_64_mix( uint64_t k1, uint64_t k2, uint64_t seed )
    {
        uint64_t h1 = seed;
        uint64_t h2 = seed;
